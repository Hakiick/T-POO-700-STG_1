...
cd /home/claude/workspace/T-POO-700-STG_1/frontend && npm run dev       # Dev server (Vite)
cd /home/claude/workspace/T-POO-700-STG_1/frontend && npm run build     # Build production
cd /home/claude/workspace/T-POO-700-STG_1/frontend && npm run prod      # Preview build
cd /home/claude/workspace/T-POO-700-STG_1/frontend && npx tsc --noEmit  # Type check
cd /home/claude/workspace/T-POO-700-STG_1/frontend && npm run lint      # Lint (si configuré)

# Le backend Elixir/Phoenix NE DOIT PAS être modifié.

# === Stabilité & Workflow ===
bash scripts/stability-check.sh       # Check complet de stabilité (exécute depuis frontend/)
...
- **Rebase only** : `git fetch origin rebuild/mobile-first && git rebase origin/rebuild/mobile-first`
- **Push** : `git push --force-with-lease origin <branch>`
- **PR** : `gh pr create --base rebuild/mobile-first`
- **Après merge** : vérifier que rebuild/mobile-first est stable
